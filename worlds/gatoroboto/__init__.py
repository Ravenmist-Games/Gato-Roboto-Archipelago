...
class GatoRobotoWorld(World):
    ...
    def create_regions(self) -> None:
        ...
        # Place locked victory item
        self.multiworld.get_location(LocationName.loc_victory, self.player).place_locked_item(
            GatoRobotoItem(ItemName.victory, ItemClassification.progression, 10999, self.player))
    ...
    def create_items(self) -> None:
        ...
        item_pool += [self.create_item(name) for name in modules_item_data_table.keys()]
        item_pool += [self.create_item(name) for name in healthkits_item_data_table.keys()]
        item_pool += [self.create_item(name) for name in cartridges_item_data_table.keys()]
        item_pool += [self.create_item(name) for name in aqueduct_events_item_data_table.keys()]
        item_pool += [self.create_item(name) for name in heater_events_item_data_table.keys()]
        item_pool += [self.create_item(name) for name in vent_events_item_data_table.keys()]

        # self.multiworld.early_items[self.player][ItemName.module_missile] = 1

        self.multiworld.itempool += item_pool
    
    def set_location_priority(self) -> None:
        self.multiworld.get_location(
            LocationName.loc_progressive_aqueducts_1, self.player).progress_type = LocationProgressType.PRIORITY
        self.multiworld.get_location(
            LocationName.loc_module_hopper, self.player).progress_type = LocationProgressType.EXCLUDED
            
    def get_filler_item_name(self) -> str:
        return self.random.choices(ItemName.Healthkits)[0]
