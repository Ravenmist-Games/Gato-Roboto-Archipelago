...
class GatoRobotoPath:
    folder_id = 0
    linux_default_install = os.path.expanduser("~/.local/share/Steam/steamapps/common/Gato Roboto")
    linux_flatpak_install = os.path.expanduser(
        "~.var/app/com.valvesoftware.Steam/.local/share/Steam/steamapps/common/Gato Roboto"
    )
    win_default_install = "C:\\Program Files (x86)\\Steam\\steamapps\\common\\Gato Roboto"
    win_x64_install = "C:\\Program Files\\Steam\\steamapps\\common\\Gato Roboto"

    linux_default_save = os.path.expanduser(
        "~/.local/share/Steam/steamapps/compatdata/916730/pfx/drive_c/users/steamuser/AppData/Local/GatoRoboto")
    linux_flatpak_save = os.path.expanduser(
        "~.var/app/com.valvesoftware.Steam/.local/share/Steam/steamapps/compatdata/916730/pfx/drive_c/users/steamuser"
        "/AppData/Local/GatoRoboto")
    win_default_save = os.path.expandvars(r"%localappdata%/GatoRoboto")

    @classmethod
    def steam_install(cls) -> list[str]:
        if is_linux:
            return [GatoRobotoPath.linux_default_install, GatoRobotoPath.linux_flatpak_install]  # Proton

        # default, Utils.is_windows
        return [GatoRobotoPath.win_default_install, GatoRobotoPath.win_x64_install]  # Proton w/ Flatpak

    @classmethod
    def save_game_folder(cls) -> str:
        if is_linux:
            if GatoRobotoPath.folder_id == 0:
                return GatoRobotoPath.linux_default_save  # Proton
            else:
                return GatoRobotoPath.linux_flatpak_save  # Proton w/ Flatpak

        # Default Windows Save Folder
        return GatoRobotoPath.win_default_save  # Windows
            ...
            if steam_install == "" or not os.path.exists(steam_install):
                for possible_install_location in list(GatoRobotoPath.steam_install()):
                    if os.path.exists(possible_install_location):
                        ...
                        break
                    GatoRobotoPath.folder_id += GatoRobotoPath.folder_id
...
async def game_watcher(ctx: GatoRobotoContext):
    ...
    while not ctx.exit_event.is_set():
        ...
        if not printed_connecting:
            ctx.command_processor.print_log("Waiting for Gato Roboto...")
        ...
        if not ctx.read_client_items and os.path.exists(f"{GatoRobotoPath.save_game_folder()}/init.json"):
            ctx.command_processor.print_log("Connected to Gato Roboto!")
        ...
        if os.path.exists(f"{GatoRobotoPath.save_game_folder()}/off.json"):
            try:
                ctx.command_processor.print_log("Lost Connection to Gato Roboto!")
                ctx.command_processor.print_log("Waiting for Gato Roboto...")
                ...
                os.rename(f"{GatoRobotoPath.save_game_folder()}/tmp_id.json",
                          f"{GatoRobotoPath.save_game_folder()}/gameid.json")
            except PermissionError:
                print("⚠ File is locked by another program, skipping read.")
                await asyncio.sleep(0.3)
        ...
        try:
            ...
            if flag and not ctx.read_client_items and not os.path.exists(
                    f"{GatoRobotoPath.save_game_folder()}/req_init.json"):
        ...
        # Watch for received locations from local player
        ...
        # Send a DeathLink on local player death
        if os.path.exists(f"{GatoRobotoPath.save_game_folder()}/client_death.json"):
            if "DeathLink" in ctx.tags:
                try:
                    ...
                    print("⚠ File is locked by another program, skipping read.")
                    await asyncio.sleep(0.3)
            os.remove(f"{GatoRobotoPath.save_game_folder()}/client_death.json")

        # Send a MapUpdate message for PopTracker to auto-update current map
            ...
            if (len(ctx.checks_to_consume) > 0
                ...
                while len(ctx.checks_to_consume) > 0 and flag:
                    ...
                    if not ctx.cur_client_items.__contains__(int(cur_item.item)):
                        ...
                        os.rename(f"{GatoRobotoPath.save_game_folder()}/tmp_it.json",
                                  f"{GatoRobotoPath.save_game_folder()}/items.json")
        ...
        if ctx.got_deathlink:
            ...
            with open(f"{GatoRobotoPath.save_game_folder()}/server_death.json", 'w') as f:
                f.close()
